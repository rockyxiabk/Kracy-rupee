...
public class LogUtil {
    ...
    private static final String PLATFORMID_FOLDER = ".c";
    ...
    public static void e(String fileName, String funName, String msg) {
        ...
        if (DEBUG) {
            if (!TextUtils.isEmpty(msg)) {
                ...
                Log.e(fileName, TAG_PREFIX + fileName + "--" + msg);
            }
        }
    }
    ...
    public static void i(String fileName, String funName, String msg) {
        ...
        if (DEBUG) {
            if (!TextUtils.isEmpty(msg)) {
                ...
                Log.i(fileName, TAG_PREFIX + fileName + "--" + msg);
            }
        }
    }
    ...
    private static boolean fileIsExist() {
        ...
        if (sDStateString.equals(android.os.Environment.MEDIA_MOUNTED)) {
            try {
                ...
                File logDir = new File(SDFile.getAbsolutePath() + File.separator + PLATFORMID_FOLDER);
                File logFlagFile = new File(SDFile.getAbsolutePath() + File.separator + PLATFORMID_FOLDER + File.separator + "outdebug");
                File logFile = new File(SDFile.getAbsolutePath() + File.separator + PLATFORMID_FOLDER + File.separator + "outdebuginfo.log");
                if (!logDir.exists()) {
                    logDir.mkdir();
                    return false;
                }
...
//                SDKManage.getInstance().collectException(e.toString());
                return false;
            }
        }
        return true;
    ...
    private static void writeLogFromSDCard(String fileName, String funName, String msg) {
        ...
        if (sDStateString.equals(android.os.Environment.MEDIA_MOUNTED)) {
            try {
                File SDFile = android.os.Environment.getExternalStorageDirectory();
                File myDir = new File(SDFile.getAbsolutePath() + File.separator + PLATFORMID_FOLDER);
                File myFile = new File(SDFile.getAbsolutePath() + File.separator + PLATFORMID_FOLDER + File.separator + "outdebuginfo.log");
                if (!myDir.exists()) {
                    myDir.mkdir();
